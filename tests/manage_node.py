...
LND_REST = os.getenv("ALICE_LND_REST")
TLS_CERT_PATH = os.getenv("ALICE_TLS_CERT_PATH")
    ...
    try:
        ...
        for peer in peers:
            ...
            node_info = requests.get(
                f"{LND_REST}/v1/graph/node/{pub_key}", headers=HEADERS, verify=CERT
            )
...
def create_channel(pubkey, local_amt_sats, private=False):
    ...
    try:
        data = {
            ...
            "local_funding_amount": str(local_amt_sats),
            "private": private,
        }
        r = requests.post(
            f"{LND_REST}/v1/channels", headers=HEADERS, json=data, verify=CERT
        )
...
def get_balance():
    try:
        print("44444")
        r = requests.get(
            f"{LND_REST}/v1/balance/blockchain", headers=HEADERS, verify=CERT
        )
...
def create_invoice(value_sat, memo="Test Invoice"):
    ...
    try:
        ...
        r = requests.post(
            f"{LND_REST}/v1/invoices", headers=HEADERS, json=data, verify=CERT
        )
...
def settle_invoice(invoice):
    ...
    try:
        ...
        r = requests.post(
            f"{LND_REST}/v1/channels/transactions",
            headers=HEADERS,
            json=data,
            verify=CERT,
        )
...
def channel_flow():
    ...
    try:
        ...
        peer_channels = [
            ch for ch in all_channels if ch["remote_pubkey"] == selected_peer["pub_key"]
        ]
        ...
        for i, ch in enumerate(peer_channels, 1):
            print(
                f"{i}. ChannelPoint: {ch['channel_point']}, Capacity: {ch['capacity']}"
            )
...
def restore_node_from_backup():
    ...
    try:
        ...
        if "multi_chan_backup" in backup_data and backup_data["multi_chan_backup"]:
            ...
            r = requests.post(
                f"{LND_REST}/v1/channels/restore",
                headers=HEADERS,
                json=payload,
                verify=CERT,
            )
...
def custom_message(selected_channel, selected_peer):
    ...
    try:
        ...
        r = requests.post(
            f"{LND_REST}/v1/custommessage", headers=HEADERS, json=data, verify=CERT
        )
...
def subscribe_msgs():
    ...
    try:
        r = requests.get(
            f"{LND_REST}/v1/custommessage/subscribe",
            stream=True,
            headers=HEADERS,
            verify=CERT,
        )
...
def get_routes(pubkey, amt):
    ...
    try:
        r = requests.get(
            f"{LND_REST}/v1/graph/routes/{pubkey}/{amt}", headers=HEADERS, verify=CERT
        )
...
def change_fee(selected_peer, selected_channel, base_fee):
    ...
    try:
        ...
        data = {
            "chan_point": {"funding_txid_str": txid, "output_index": output_index},
            "base_fee_msat": base_fee,
            "time_lock_delta": 144,
        }
        r = requests.post(
            f"{LND_REST}/v1/chanpolicy", json=data, headers=HEADERS, verify=CERT
        )
...
def list_towers():
    try:
        r = requests.get(
            f"{LND_REST}/v2/watchtower/client", headers=HEADERS, verify=CERT
        )
...
def add_towers(pubkey):
    ...
    try:
        ...
        r = requests.post(
            f"{LND_REST}/v2/watchtower/client", json=data, headers=HEADERS, verify=CERT
        )
...
def get_tower_stats():
    ...
    try:
        r = requests.get(
            f"{LND_REST}/v2/watchtower/client/stats", headers=HEADERS, verify=CERT
        )
...
def deactivate_tower(pubkey):
    ...
    try:
        ...
        r = requests.post(
            f"{LND_REST}/v2/watchtower/client/tower/deactivate/{pubkey_urlsafe}",
            json=data,
            headers=HEADERS,
            verify=CERT,
        )
...
def remove_tower(pubkey):
    ...
    try:
        ...
        r = requests.delete(
            f"{LND_REST}/v2/watchtower/client/{pubkey_urlsafe}",
            headers=HEADERS,
            verify=CERT,
        )
...
def get_tower_info():
    ...
    try:
        r = requests.get(
            f"{LND_REST}/v2/watchtower/server", headers=HEADERS, verify=CERT
        )
...
def list_payments():
    try:
        ...
        # r = requests.get(f"{LND_REST}/v1/channels/transactions", headers=HEADERS, verify=CERT)
    ...
    try:
        ...
        if choice == "1":
            ...
            if choice == "1":
                ...
                pubkey = input(
                    "Enter the public key of the tower you want to deactivate: "
                )
